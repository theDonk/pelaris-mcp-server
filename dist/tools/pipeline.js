...
export function registerPipelineTools(server) {
    server.tool("list_pipeline_items", "List content pipeline items, optionally filtered by status or type.", {
        status: z
            .string()
            .optional()
    ...
    server.tool("get_pipeline_item", "Get a single content pipeline item by ID.", {
        ...
        try {
            ...
            if (!doc.exists) {
                return {
                    content: [{ type: "text", text: `Pipeline item "${id}" not found` }],
                    isError: true,
                };
            }
            return {
                content: [
                    { type: "text", text: JSON.stringify({ id: doc.id, ...doc.data() }, null, 2) },
                ],
            };
        }
        catch (error) {
    ...
    server.tool("write_pipeline_item", "Create or update a content pipeline item. Provide an ID to update, omit to create.", {
